...
def build_setup_embed(st: dict) -> discord.Embed:
    day = "今日" if st.get("day") == "today" else "明日" if st.get("day") == "tomorrow" else "今日（未変更）"
    start = st.get("start") or "--:--"
    end = st.get("end") or "--:--"
    interval = f"{st.get('interval')}分" if st.get("interval") else "未設定（必須）"
    notify = f"<#{st.get('notify_channel_id')}>" if st.get("notify_channel_id") else "未設定（必須）"
    everyone = "ON" if st.get("everyone") else "OFF"
    title = st.get("title") or "未設定（任意）"
    step = st.get("step", 1)

    e = discord.Embed(title=f"🧩 募集パネル作成（{step}/2）", color=0x5865F2)
    ...
    if step == 2:
        e.add_field(name="間隔", value=interval, inline=True)
        e.add_field(name="通知チャンネル", value=notify, inline=True)
        e.add_field(name="@everyone", value=everyone, inline=True)
        e.add_field(name="タイトル", value=title, inline=False)

        e.set_footer(text="2/2: 間隔・通知・タイトル → 作成（通知チャンネルは必須）")
    else:
        e.set_footer(text="1/2: 時刻を選ぶ → 次へ（今日がデフォルト）")
...
def _everyone_button(everyone_on: bool) -> discord.ui.Button:
    if everyone_on:
...
class SetupWizardViewStep1(discord.ui.View):
    def __init__(self, st: dict):
        ...
        self.add_item(_everyone_button(bool(st.get("everyone"))))
...
class TitleModal(discord.ui.Modal):
    def __init__(self, current: str | None = None):
        super().__init__(title="募集パネルのタイトル", custom_id="setup:titlemodal")
        self.title_input = discord.ui.TextInput(
            label="タイトル（空でもOK）",
            placeholder="例）夜の募集 / 練習部屋 / 参加者募集 など",
            default=current or "",
            required=False,
            max_length=60,
        )
