print("🔥 BOOT MARKER v2026-02-27 B-mode STABLE ULTIMATE 🔥")
...
class MyClient(discord.Client):
    ...
    async def setup_hook(self):
        ...
        # ✅ setup_hook で sync（これが一番安定）
        try:
            await self.tree.sync()
            self._synced = True
            print("✅ commands synced (setup_hook)")
    ...
    async def on_interaction(self, interaction: discord.Interaction):
        """
        ✅ 超重要：
        - component はここで custom_id を処理（B方式）
        - それ以外（スラッシュ等）は tree.process_interaction に渡す（公式・これで CommandNotFound 消える）
        """
        try:
            # 1) ボタン/セレクト（component）
            if interaction.type == discord.InteractionType.component:
                ...
                if custom_id.startswith("panel:breaktoggle:"):
                    ...
                    # ✅ 管理者 or 管理ロール
                    allowed = await self.dm.is_manager(interaction)
                    if not allowed:
                        await safe_send(interaction, "❌ 管理者/管理ロールのみ実行できます", ephemeral=True)
                        return
            ...
            # 2) スラッシュ等（✅これが無いと CommandNotFound になる）
...
async def main():
    ...
    # ✅ Render は勝手に再起動するので while True で回さない
