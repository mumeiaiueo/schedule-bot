...
class DataManager:
    ...
    async def render_panel(self, bot: discord.Client, panel_id: int):
        ...
        for r in slots[:25]:
            ...
            if r.get("is_break"):
                dot, mention, style, disabled = "⚪", "", discord.ButtonStyle.secondary, True
            elif r.get("reserver_user_id"):
                dot, mention, style, disabled = "🔴", f" <@{r['reserver_user_id']}>", discord.ButtonStyle.danger, False
            else:
                dot, mention, style, disabled = "🟢", "", discord.ButtonStyle.success, False
        ...
        should_mention = bool(panel.get("everyone", False)) and not bool(panel.get("everyone_mentioned", False))
        content = "@everyone" if should_mention else None

        msg = await channel.send(content=content, embed=embed, view=view)

        def work_update_mid():
            upd = {"panel_message_id": str(msg.id)}
            if should_mention:
                upd["everyone_mentioned"] = True
            sb.table("panels").update(upd).eq("id", panel_id).execute()
