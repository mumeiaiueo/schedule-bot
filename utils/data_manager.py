...
class DataManager:
    ...
    async def create_panel(
        ...
        created_by: str,
        everyone: bool = False,  # ✅ missing防止（呼び出し側が渡してもOK）
    ):
        self._require_db()
    ...
    async def render_panel(self, bot: discord.Client, panel_id: int):
        ...
        view = PanelView(panel_id, buttons, notify_paused=notify_paused)
    ...
    async def toggle_notify_paused(self, panel_id: int) -> bool:
        ...
        def work_get():
            rows = sb.table("panels").select("notify_paused").eq("id", panel_id).limit(1).execute().data or []
            if not rows:
                return None
            return bool(rows[0].get("notify_paused", False))

        cur = await self._db(work_get)
        if cur is None:
        ...
        await self._db(work_set)
        return new_val

    # -----------------------------
    # 3min reminders (send to panel channel)
    # -----------------------------
    async def send_3min_reminders(self, bot: discord.Client):
        ...
        for slot in rows:
            ...
            notify_ch = None
            try:
                notify_ch = bot.get_channel(int(panel["channel_id"]))
            except Exception:
                notify_ch = None
            if not notify_ch:
                continue
